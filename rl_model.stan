...
model {
  ...
  for (i in 1:N) {
    ...
    // Initialize values
    ev1 = initV; // initial ev1 values (value of correct stimulus)
    ev2 = initV; // initial ev2 values (value of wrong stimulus)
    ew = initV; // initial ew values
    
    for (t in 1:Tsubj[i]) {
      // Softmax choice
      choice[i, t] ~ categorical(ev1 * beta[i]/(ev1 * beta[i]+ ev2 * beta[i]));
      ...
      {
        ev1[choice[i, t]] *= (1-alpha[i]) * ewt1;
        ev1[choice[i, t]] += outcome[i, t];
        ev1[choice[i, t]] /= ew[choice[i, t]];
        ev2[choice[i, t]] *= (1-alpha[i]) * ewt1;
        ev2[choice[i, t]] += (1-outcome[i, t]);
        ev2[choice[i, t]] /= ew[choice[i, t]];
      }
    }
  }
}
...
generated quantities {
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      // Initialize values
      ev1 = initV; // initial ev values
      ev2 = initV; // initial ev values
      ew = initV; // initial ew values
      ...
      for (t in 1:Tsubj[i]) {
        // Softmax choice
        log_lik[i] += categorical_lpmf(ev1 * beta[i]/(ev1 * beta[i]+ ev2 * beta[i]));
        
        // generate posterior prediction for current trial
        y_pred[i, t] = categorical_rng(ev1 * beta[i]/(ev1 * beta[i]+ ev2 * beta[i]));
